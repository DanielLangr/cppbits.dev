...
To understand *{cpp}*, we first need to understand *programming*. Programming is basically a way how to *express some functionality* in a specific *human-“readable” form*, which differes from language to language. We then want this written funtionality -- called *source code* -- to be *performed on a computer system*. More precisely, performed by its *CPU* (we will not cover here special cases such as GPU programming).
...
These are the numbers (called _opcodes_) my CPU undestands. We can undrstand them as well by studying the _Intel Instruction Set Reference_ or similar sources. For instance, we can read there that the last `c3` opcode corresponds to the `ret` instruction.

However, let's get back to programming languages. *How do they differ?* Among others -- such as that they use a different source code form/syntax -- each language proivdes a *different set of abstraction mechanisms*. An *abstraction mechanism* is a *programming language entity/concept* that allows us to *express some desired functionality*. Exemplary abstraction mechanisms are _variables, functions (with parameters and return values), loop constructs, structures, classes, inheritance, polymorphism, templates, reflection,_ and many more.

The important aspect of abstracion mechanisms is that they *do not exist at a machine code level*, that is, they *do not exist at runtime*. If we decoded the above-presented machine code, we would realize that it contained only the insturctions for register manipulations and transfering control back to a function caller.
