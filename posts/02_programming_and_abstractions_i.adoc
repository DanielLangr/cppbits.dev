...
These are the numbers (called _opcodes_) my CPU undestands. We can undrstand them as well by studying the _Intel Instruction Set Reference_ or simnilar sources. For instance, we can read there that the last `c3` opcode corresponds to the `ret` instruction.
...
The important aspect of abstracion mechanisms is that *do not exist at a machine code level*, that is, they *do not exist at runtime*. If we decoded the above-presented machine code, we would realize that it contained only the insturctions for register manipulations and transfering control back to a function caller.
