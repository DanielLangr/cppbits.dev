...
{cpp} itself -- defined by the {cpp} standard -- *doesn't have the concept of stack*. From the perspective {cpp}, values of function local variables *cannot be stored at a place it does not recognize*. Yet, all today's mainstream CPU architectures are stack-based and {cpp} implementations reflect it. Can we say at least that *on a typical modern computer system, values of function local variables are stored on the stack*? No, even this is *generally wrong*. Consider the following function:
...
We can clearly see that the values of `a`,`b`, and `c` variables *did not at all exist at runtime*. Not only they were *not stored on the stack*, they were even not stored anywhere else. Generally, variable values *may be stored on the stack*, but this was not the case.
...
To understand what happened here we need to understand some of the key {cpp} mechanisms -- *the as-if rule and the observable behavior concept*. We will get to them later in this tutorial.
