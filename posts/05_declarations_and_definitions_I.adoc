...
Finally, why are there those two instructions that manipulate the _stack pointer_ register `rsp`? It is again related to the applied ABI. Namely, the ABI prescribes that when `call` instruction is executed, `rsp` needs to be aligned to a 16-byte address. This holds when `main` is called, but the corresponding `call` instruction decreases `rsp` by 8 (by storing the value of the _instruction pointer_ `rip` on the stack). To ensure the 16-byte alignment of `rsp` before `call add` is executed inside `main`, `rsp` therefore needs to be decreased by 8 once more (`sub rsp, 8`). After the `add` function call, the effect of this operation is reverted by `add rsp, 8`.
****
