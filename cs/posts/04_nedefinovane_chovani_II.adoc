...
Nedefinované chování je velmi nebeznečné, především z důvodu, že program vygenerovaný z příslušného zdrojového kódu se často *chová zdánlivě „očkávaným“ způsobem*. Typickým argumentem v diskuzích je, že „když se program chová dle očekávání, tak jeho zdrojový kód přece musí být v pořádku“. Nic nemůže být vzdálenější skutenčosti.
...
Podle standradu {cpp} má proměnná `i` díky absenci inicializace *neurčitou hodnotu*. Její *vyhodnocení* v rámci `return i;` poté vede na nedefinované chování. Pokud však budeme předpokládát, že implmentace {cpp} zdrojový kód přeloží a vygeneruje spustitelný program, jaké bude jeho *očekávané chování*? Jsou zde *dvě základní možnosti*:
...
Formulace první otázky je ve skutečnosti *zavádějící*. Podle definice nedefinovaného chování bychom se totiž správně měli ptát, proč stadndard {cpp} definuje chování program *pouze pokud jsou splněna určitá pravidla*. Jinak řečeno, proč nedefinuje chování programu *za všech okolností*.
...
Jinými slovy se dá říct, že koncept nedefinovaného chování *umožňuje implementaci zaměřit se na co nejvyšší výkon a efektivitu*. K dosažení tohto cíle ale musí mít možnost předpokládat, že *jsou splněna pravidla* obsažená ve standardu {cpp}, a ostatní situace neřešit.

Nyní nám zbývá druhá otázka, a sice, zda lze nedefinované chování *odhalit*. Obecně to bohužel *monžné není*. Pro příklad předpokládejme, že kromě výše ukázaného zdrojového souboru `lib.cpp` máme ještě následující druhý zdrojový soubor:
...
_Poznámka:_ Poslední dva kroky lze efektivně spojit do jednoho příkazu `g++ -O2 prog.cpp lib.o`. Na funkcionalitu překaldu a spojení to nemá žádný vliv.

Otázkou nyní je, zda impelemtnace vůbec může odhalit nedefinované chování způsobené dereferencí neplatného ukazatele (ukazatele na již neexistující objekt). Rozeberme si to v souvislosti s výše definovanými kroky:
...
- Při spojování *porušení pravidel standardu {cpp} již ohladit nelze*, protože tato pravidla se týkají jen zdrojového kódu. A ta už _linker_ „nevidí“.
